<template>
  <q-page>
              ...
              <q-card style="min-width: 400px" class="rounded-borders">
                ...
                <q-card-section>
                  <q-input
                    ...
                    outlined
                    dense
                    class="q-mb-md shadow-8"
                  />
                  <q-select
                    v-model="newTeamMembers"
                    :options="authStore.getUsers"
                    option-value="id"
                    option-label="name"
                    multiple
                    emit-value
    ...
    <q-dialog v-model="teamMembersDialog" persistent>
      <q-card style="min-width: 400px; border-radius: 20px">
        <q-card-section>
          <div class="text-h6 text-center text-purple-8 q-mb-sm">
            Team Members for "{{ selectedTeam?.name }}"
          </div>
        </q-card-section>

        <q-card-section>
          <q-list bordered separator>
            <q-item v-for="member in selectedTeam?.members" :key="member.id">
              <q-item-section>
    ...
    <q-dialog v-model="manageMembersDialog" persistent>
      <q-card style="min-width: 400px; border-radius: 20px">
        ...
        <q-card-section>
          <div class="q-mb-md">
            <div class="text-subtitle1 q-mb-xs">Current Members</div>
            <q-list bordered separator>
              <q-item v-for="member in selectedTeam?.members" :key="member.id">
                <q-item-section>
                  <q-item-label>{{ member.name }}</q-item-label>
                ...
                <q-item-section side>
                  <q-btn
                    ...
                    icon="person_remove"
                    color="red-6"
                    @click="removeMember(selectedTeam.id, member.id)"
                  />
                </q-item-section>
              </q-item>
            </q-list>
...
const saveTeam = async () => {
  ...
  try {
    await teamStore.createTeam(newTeamName.value, newTeamMembers.value);
    newTeamName.value = "";
...
const availableUsers = computed(() => {
  const currentMemberIds = new Set(
    selectedTeam.value?.members.map((m) => m.id)
  );
...
const addMembers = async () => {
  if (newMembersToAdd.value.length === 0) {
    $q.notify({ type: "info", message: "No new members selected." });
    return;
  }
  try {
    for (const userId of newMembersToAdd.value) {
      await teamStore.addMemberToTeam(selectedTeam.value.id, userId);
    }
    await teamStore.fetchTeams();
    closeManageDialog();
    $q.notify({ type: "positive", message: "Members added successfully!" });
  } catch (err) {
    $q.notify({ type: "negative", message: "Failed to add members." });
  }
};

const removeMember = async (teamId, userId) => {
  try {
    await teamStore.removeMemberFromTeam(teamId, userId);
    await teamStore.fetchTeams();
    $q.notify({ type: "positive", message: "Member removed successfully!" });
  } catch (err) {
